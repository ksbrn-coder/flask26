# pip install flask
# 플라스크란? : 파이썬으로 만든 db 연동 콘솔 프로그램을 웹으로 연결하는 프레임워크
...
#                플라스크    프론트 연결     요청, 응답 주소 전달  주소 생성  상태 저장
from LMS.common import Session
from LMS.domain import Board

app = Flask(__name__)
app.secret_key = 'ASAFAAFDSFF134'
# 세션을 사용하기 위해 보안키 설정 (아무 문자열이나 입력)

@app.route('/login', methods=['GET','POST']) # http://localhost:5000/login
    # methods 는 웹에 동작을 관여한다.
    # GET : URL 주소로 데이터를 처리(보안상 좋지 않음, 빠름)
    # POST : BODY 영역에서 데이터를 처리(보안상 좋음, 대용량에서 많이 사용함)
    # 대부분 처음에 화면(HTML 렌더)을 요청할 때는 GET 방식으로 처리
    # 화면에 있는 내용을 백엔드로 전달할 때는 POST 방식으로 처리

def login():
    if request.method == 'GET': # 처음 접속하면 get 방식으로 화면 출력용
    ...
    try: # 예외발생 가능성 있음
        with conn.cursor() as cursor: # db에 커서객체 사용
            ...
            if user:
                ...
                # 처리 후 이동하는 경로 http://localhost:/index로 감 (get 방식)
            else:
                # 찾은 계정이 없다.
...
@app.route('/logout') # 기본 동작이 get 방식이라 methods=['GET'] 생략 가능
def logout():
    session.clear()
    return redirect(url_for('login')) # http://localhost:5000/login (get 방식)

@app.route('/join', methods=['GET', 'POST']) # 회원가입용 함수
def join(): # http://localhost:5000/ get 메서드(화면 출력) post(화면폼 처리용)
    if request.method == 'GET':
        return render_template('join.html') # 로그인화면용 프론트로 내보냄

    # POST 메서드인 경우 (폼으로 데이터가 넘어올 때 처리)
    ...
    try:
        with conn.cursor() as cursor:
            if request.method == 'GET':
                ...
                #                       가장 중요한 포인트  get 요청시 페이지     객체 전달용 코드
...
@app.route('/mypage') # http://localhost:5000/mypage get 요청 시 처리됨
def mypage():
    ...
    try:
        with conn.cursor() as cursor:
            ...
            # 2. 내가 쓴 게시글 갯수 조회 (작성하신 boards 테이블 활용)
